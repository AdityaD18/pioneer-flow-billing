class TallyXMLBuilder:
    """Builds dedicated, structured TDL XML requests for Tally Prime 7.1."""
  ...
  <BODY>
    ...
    def build_stock_items_request(min_alter_id=None):
        alter_filter = f"<FILTER>AlterIdFilter</FILTER>" if min_alter_id else ""
        system_filter = f"""<SYSTEM TYPE="Formulae" NAME="AlterIdFilter">$$Number:$AlterId &gt; {min_alter_id}</SYSTEM>""" if min_alter_id else ""
